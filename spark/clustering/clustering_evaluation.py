...
def log_to_db(
    ...
    cur.execute(
        """
        INSERT INTO clustering_audit (
            cluster, model, run_datetime, params, inference_time,
...
def evaluation():
    ...
    val_df = val_df.join(clusters_df, on="customer", how="inner")
    test_df = test_df.join(clusters_df, on="customer", how="inner")
    ...
    # ====================== Evaluate all models per cluster ======================
    target_col = "cons_h"
    ...
    for cluster in clusters:
        ...
        cluster_models_path = os.path.join(
            models_path, "clustering", f"cluster_{cluster}"
        )

        cluster_train_df = train_df.filter(train_df.cluster == cluster)
        cluster_val_df = val_df.filter(val_df.cluster == cluster)
        cluster_test_df = test_df.filter(test_df.cluster == cluster)

        for name in os.listdir(cluster_models_path):
            if (
                not os.path.isdir(os.path.join(cluster_models_path, name))
                or name == "best_model"
            ):
                continue
            ...
            model_path = os.path.join(cluster_models_path, name)
            model = PipelineModel.load(model_path)

            # Training metrics
            train_predictions = model.transform(cluster_train_df)
            ...
            # Validate
            val_predictions = model.transform(cluster_val_df)
            ...
            test_predictions = model.transform(cluster_test_df)
