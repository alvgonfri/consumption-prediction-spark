...
def model_selection():
    ...
    clusters = [
        int(d.split("_")[1])
        for d in os.listdir(clustering_models_dir)
    ...
    for cluster in clusters:
        ...
        df = pl.read_database(
            f"SELECT * FROM clustering_audit WHERE cluster = {cluster} ORDER BY run_datetime DESC LIMIT 4",
            conn,
        )  # Get the 4 models from the latest run for this cluster
        conn.close()
        ...
        df = df.with_columns(
            [
                normalize(df["rmse_test"], higher_is_better=False).alias(
                    "rmse_test_norm"
                ),
                normalize(df["mae_test"], higher_is_better=False).alias(
                    "mae_test_norm"
                ),
        ...
        print("\n=== Model scores for cluster", cluster, "===")
        print(
            df.select(
                [
                    "model",
        ...
        print("\n=== Best model selected for cluster", cluster, "===")
